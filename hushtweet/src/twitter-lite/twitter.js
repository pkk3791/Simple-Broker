...
class Twitter {
  ...
  async getRequestToken(twitterCallbackUrl) {
    ...
    if (parameters) requestData.url += '?' + stringify(parameters);
  ...
  async getAccessToken(options) {
    ...
    const parameters = { oauth_verifier: options.oauth_verifier, oauth_token: options.oauth_token };
    if (parameters.oauth_verifier && parameters.oauth_token) requestData.url += '?' + stringify(parameters);
  ...
  _makeRequest(method, resource, parameters) {
    ...
    if (parameters)
      ...
      else requestData.url += '?' + stringify(parameters);
  ...
  post(resource, body) {
    ...
    if (JSON_ENDPOINTS.includes(resource)) {
      ...
      body = percentEncode(stringify(body));
  ...
  stream(resource, parameters) {
    ...
    const request = Fetch(requestData.url, {
      ...
      body: percentEncode(stringify(parameters)),
    });
